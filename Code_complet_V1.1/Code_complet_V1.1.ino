...
const char* ap_password= "achlabete"; //Access Point Password
...
void handleRequest() { //Modifier car ça ne fonctionne pas pour l'instant
  if (server.uri() == "/add_finger") {
    add_finger();
    server.send(200, "text/plain", "Empreinte digitale ajoutée");
  }else if (server.uri() == "/match_finger") {
    match_finger();
    server.send(200, "text/plain", "Empreinte digitale trouvée");
  }else if (server.uri() == "/delete_finger") {
    delete_finger();
    server.send(200, "text/plain", "Empreinte digitale supprimée");
  }
}
...
void loop(){
  ...
  {
    Serial.print("New Device Connected to the ESP8266 wifi");
    current_stations=new_stations;
  }
   
  if(current_stations>new_stations)//Device is Disconnected
  {
    Serial.print("Device disconnected from the ESP8266 wifi");
    current_stations=new_stations;
  }
  ...
  if(current_stations>0){
    ...
    Serial.println("Veuillez vous connecter au wifi 'ESP8266' à l'aide du mot de passe 'achlabete'\n");
    delay(5000);
  }
}
